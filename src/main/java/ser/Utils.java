...
public class Utils {
    ...
    public static String exportDocuments(IDocument document, String exportPath) throws IOException {
        String expt = "";
        ...
        for(int representationConter = 0; representationConter < document.getRepresentationCount(); ++representationConter) {
            for(int partDocumentCounter = 0; partDocumentCounter < document.getPartDocumentCount(representationConter); ++partDocumentCounter) {
                ...
                try {
                    ...
                    if (fde.getFDEType() == 3) {
                        expt = exportPath + "/" + ((IFileFDE)fde).getLongFileName();
    ...
    public static String getCategoryIDFromGVlist(String name) throws Exception {
        String rtrn = "";
        IStringMatrix settingsMatrix = server.getStringMatrixByID("DocumentClasses", session);
    ...
    public static String getCategoryFolderNameIDFromGVlist(String name) throws Exception {
        String rtrn = "";
        IStringMatrix settingsMatrix = server.getStringMatrix("GibCategories", session);
        if(settingsMatrix!=null) {
            for (int i = 0; i < settingsMatrix.getRowCount(); i++) {
                String rowID = settingsMatrix.getValue(i, 0);
                String rowName = settingsMatrix.getValue(i, 1);
                String rowFolderName = settingsMatrix.getValue(i, 3);
                if (rowName.equalsIgnoreCase(name)) {
                    rtrn = rowFolderName;
                    break;
                }
    ...
    public static String getCustomerRecordFromGVlist(String name) throws Exception {
        String rtrn = "";
        IStringMatrix settingsMatrix = server.getStringMatrix("GibCategories", session);
        if(settingsMatrix!=null) {
            for (int i = 0; i < settingsMatrix.getRowCount(); i++) {
                String rowID = settingsMatrix.getValue(i, 0);
                String rowName = settingsMatrix.getValue(i, 1);
                if(Objects.equals(rowName, name)){
                    if(Objects.equals(rowID, "corp")){
                        rtrn = Conf.RecordTypesID.corp;
                        break;
                    }
                    if(Objects.equals(rowID, "retail")){
                        rtrn = Conf.RecordTypesID.retail;
                        break;
                    }
                }
            }
    ...
    static IInformationObject getCustomerDoc(IDocument doc, String categoryID, ProcessHelper helper) {
        String category = doc.getDescriptorValue(Conf.Descriptors.Categories);
        String accountNr = doc.getDescriptorValue(Conf.Descriptors.AccountNumber);
        String CIF = doc.getDescriptorValue(Conf.Descriptors.CIF);
        StringBuilder builder = new StringBuilder();
        builder.append("TYPE = '").append(categoryID).append("'")
                .append(" AND ")
                .append(Conf.DescriptorLiterals.AccountNumber).append(" = '").append(accountNr).append("'")
                .append(" AND ")
                .append(Conf.DescriptorLiterals.CIF).append(" = '").append(CIF).append("'");
        String whereClause = builder.toString();
        log.info("Where Clause: " + whereClause);
        IInformationObject[] informationObjects = helper.createQuery(new String[]{Conf.Databases.GIBDocument}, whereClause , "",1, true);
        if(informationObjects.length < 1) {return null;}
        return informationObjects[0];
    }
    static IFolder getCustomerFolder(IDocument doc, ProcessHelper helper) {
        String category = doc.getDescriptorValue(Conf.Descriptors.Categories);
        String accountNr = doc.getDescriptorValue(Conf.Descriptors.AccountNumber);
        String CIF = doc.getDescriptorValue(Conf.Descriptors.CIF);
        StringBuilder builder = new StringBuilder();
        builder.append("TYPE = '").append(Conf.ClassIDs.CustomerCorpRec).append("'")
                .append(" AND ")
                .append(Conf.DescriptorLiterals.CIF).append(" = '").append(CIF).append("'");
        String whereClause = builder.toString();
        log.info("Where Clause: " + whereClause);
        IInformationObject[] informationObjects = helper.createQuery(new String[]{Conf.Databases.GIBDRecord}, whereClause , "",1, true);
        if(informationObjects.length < 1) {
            StringBuilder builder2 = new StringBuilder();
            builder2.append("TYPE = '").append(Conf.ClassIDs.CustomerRetailRec).append("'")
                    .append(" AND ")
                    .append(Conf.DescriptorLiterals.CIF).append(" = '").append(CIF).append("'");
            String whereClause2 = builder2.toString();
            log.info("Where Clause22: " + whereClause2);
            informationObjects = helper.createQuery(new String[]{Conf.Databases.GIBDRecord}, whereClause2 , "",1, true);
        }
        if(informationObjects.length < 1) {return null;}
        return (IFolder) informationObjects[0];
    }
    public static boolean addToNode(IFolder folder, IDocument pdoc) throws Exception {
        log.info("Add2Node Start");
        boolean add2Node = false;
        String category = pdoc.getDescriptorValue(Conf.Descriptors.Categories);

        String classID = pdoc.getClassID();
        String className = pdoc.getArchiveClass().getName();
        String folderName = Utils.getCategoryFolderNameIDFromGVlist(className);

        int nodesCount = folder.getNodes2().getCount();
        INodes nodesByName = folder.getNodes2();
        INode custDocNode = null;
        for(int i=0;i<nodesCount;i++){
            INode docNode = nodesByName.getItem(i);
            if(Objects.equals(docNode.getName(), folderName)){
                custDocNode = docNode;
                break;
            }
        }
        if(custDocNode == null){
            throw new Exception("Add2Node..folder not found." + folderName);
        }

        custDocNode.refresh(true);
        boolean isExistElement = false;
        log.info("Start ProjectDoc exit in folder: " + isExistElement);
        IElements nelements = custDocNode.getElements();
        ...
        log.info("Finish Add2Node exit in folder: " + isExistElement);
        if(!isExistElement) {
            add2Node = folder.addInformationObjectToNode(pdoc.getID(), custDocNode.getID());
            log.info("Add2Node add to folder: " + custDocNode.getID());
            if (add2Node) {
                //pdoc.setDescriptorValue("ccmLinkedFolderID", newNode.getID());
                //pdoc.commit();
                log.info("Add2Node setting new node ID: " + custDocNode.getID());
            }
            folder.commit();
        }
        return add2Node;
    }
    public static IDocument newFileToDocumentClass(String filePath, String archiveClassID) throws Exception {
    ...
    public IArchiveFolderClass createNewCustomerCorpRecord(){
        try{
            log.info("Creating new MDP record..");
            IMetaDataManager dataManager = Utils.server.getMetaDataManager(Utils.session);
            IArchiveFolderClassModifiable archiveFolderClassModifiable = dataManager.createArchiveFolderClass(Conf.ClassIDs.CustomerCorpRec);
    ...
    public static IFolder newCustomerFolderFromDoc(IDocument doc) throws Exception{
        try{
            String classID = doc.getClassID();
            String className = doc.getArchiveClass().getName();
            String customerRecordID = Utils.getCustomerRecordFromGVlist(className);
            IFolderConnection folderConnection = Utils.session.getFolderConnection();
            IFolder rtrn = folderConnection.createFolder();
            IArchiveFolderClass afc = Utils.server.getArchiveFolderClass(customerRecordID, Utils.session);
            ...
            log.error("Exception caught...newCustomerFolder..:" + e.getMessage());
            return null;
        }
    }
}
